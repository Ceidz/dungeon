...
function GMGame(){
    ...
    function setupAuthListener(){
      ...
      unsub=_fbAuth.onAuthStateChanged(user=>{
        ...
        if(user){
          ...
          if(user.uid===GM_UID)setIsGMMode(true);
          ...
          setAuthUser(null);setMyUid(null);setIsGMMode(false);setScreen("auth");
        }
      });
    }
  ...
  var doLogout=useCallback(()=>{
    ...
    setScreen("auth");setMe(null);setMyUid(null);setAuthUser(null);setIsGMMode(false);
    setEncounter(null);setDungeon(null);dungeonRef.current=null;
    setCombatTurn("player");setShielded(false);setPoisoned(false);setStunned(false);
    setRageTurns(0);setHasteBonus(0);setExploring(false);setResting(false);
  ...
  // Enter game (called after char creation or load)
  var enterGame=useCallback((charData,slot,isLoad)=>{
    ...
    setTimeout(()=>{
      ...
      gLog("One global server. All players share the same world.","story");
    },80);
    // Only show tutorial for new characters, and only if they've never seen it
    var tutKey='eth_tut_'+(myUid||'local');
    var alreadyDone=localStorage.getItem(tutKey)==='1';
    if(isLoad||alreadyDone){
      setTutorialStep(-1);setTutorialDone(true);
    }else{
      setTutorialStep(0);setTutorialDone(false);
    }
  },[myUid,gLog]);

  var finishTutorial=useCallback(()=>{
    setTutorialStep(-1);setTutorialDone(true);
    var tutKey='eth_tut_'+(myUid||'local');
  ...
  return(
    ...
    React.createElement('style', null, `
        ...
        @keyframes combatFlash{0%{background:rgba(255,85,51,0.15)}100%{background:transparent}}
        button:hover{opacity:0.82!important;transform:translateY(-1px);}
        button:active{transform:translateY(0)!important;}
        button{transition:all 0.15s ease!important;}
        ::-webkit-scrollbar{width:3px;height:3px;}::-webkit-scrollbar-thumb{background:#ffffff18;border-radius:10px;}
        ::-webkit-scrollbar-track{background:transparent;}
        .gm-side::-webkit-scrollbar-thumb{background:#1a2535;}
        *{-webkit-tap-highlight-color:transparent;box-sizing:border-box;}
        button{touch-action:manipulation;}
        input,select{background:rgba(255,255,255,0.06)!important;border:1px solid rgba(255,255,255,0.12)!important;color:#c8d0dc!important;border-radius:7px!important;outline:none!important;font-family:Georgia,serif!important;}
        input::placeholder{color:#3a4050!important;}
        .nav-pill{transition:all 0.2s;}
        .nav-pill.active{background:rgba(255,255,255,0.1)!important;border-color:${regData.accent}!important;color:${regData.accent}!important;}
        .cls-card:hover{transform:translateY(-2px);border-color:${regData.accent}88!important;}
        ...
        @media(max-width:700px){
          ...
          .cls-grid{grid-template-columns:repeat(3,1fr)!important;}
          .combat-btns{flex-direction:column!important;}
          .combat-btn-row{flex-direction:row!important;}
        }
        @media(max-width:900px){
          .split-view{flex-direction:column!important;}
          .gm-panel{max-height:55vh!important;}
        }
      `)
      ...
      , tutorialStep>=0&&tutorialStep<TUTORIAL_STEPS.length&&!tutorialDone&&React.createElement('div', { style:{position:"fixed",inset:0,background:"rgba(0,0,0,0.85)",zIndex:20000,display:"flex",alignItems:"flex-end",justifyContent:"center",padding:"0 0 16px 0"},}
          , React.createElement('div', { style:{maxWidth:540,width:"calc(100% - 28px)",background:"#0f0c08",border:"2px solid #e8aa44",borderRadius:10,padding:"20px 20px",animation:"slideUp 0.3s ease",touchAction:"manipulation"},}
            ...
            , React.createElement('div', { style:{display:"flex",gap:8}},
                React.createElement('button', { onClick:function(){finishTutorial();}, style:{flex:1,background:"rgba(255,255,255,0.03)",border:"1px solid #2a1e0a",color:"#5a4030",borderRadius:4,padding:"9px 0",fontSize:10,cursor:"pointer",fontFamily:"Georgia,serif",touchAction:"manipulation"}}, "Skip"),
      ...
      , React.createElement('div', { className: "split", style: {display:"flex",minHeight:"100vh"},}
        , React.createElement('div', { style: {flex:1,minWidth:0,display:"flex",flexDirection:"column",borderRight:"1px solid #2a1e0a"},}
          , React.createElement('div', { style: {background:"rgba(0,0,0,0.55)",borderBottom:"1px solid #2a1e0a",padding:"10px 16px",display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"},}
            ...
            , myUid===GM_UID&&React.createElement(Btn, { onClick: ()=>{setIsGMMode(v=>!v);setGmOpen(v=>!v);}, style: {background:isGMMode?"rgba(61,214,140,0.15)":"rgba(61,214,140,0.05)",border:"1px solid "+(isGMMode?"#3dd68c":"#3dd68c44"),color:isGMMode?"#3dd68c":"#3dd68c66",padding:"5px 10px",borderRadius:3,fontSize:11,fontFamily:"monospace",letterSpacing:1},}, isGMMode?"◈ GM":"◈")
          )
        ...
        /* ── GM PANEL (Rebuilt) ─────────────────────────── */
        , myUid===GM_UID&&React.createElement('div', {style:{position:"fixed",bottom:16,right:16,zIndex:8000}},
          /* Toggle button */
          React.createElement(Btn, {
